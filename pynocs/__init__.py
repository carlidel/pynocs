...
class nocs_tools(object):
    ...
    def get_tracking_data(self):
        ...
        df = pd.DataFrame(data, columns=['id', 'mass', 'energy', 'x', 'y', 'v_x', 'v_y'])
        return df
    ...
    def load_simulation_photo(self, df, tracking_lambda=lambda x:False):
        ...
        tracking_lambda : lambda, optional
            since the tracking information is lost in this passage, you can rebuild it by passing a lambda function with the desired behaviour implemented, this lambda takes as argument a tuple containing the following data: tuple('mass', 'radius', 'energy', 'x', 'y', 'v_x', 'v_y'), by default lambda tuple : False
        """        
        for index, row in df.iterrows():
            self.add_sphere(row['x'], row['y'], row['radius'], row['mass'], row['v_x'], row['v_y'], tracking_lambda(
                (row['x'], row['y'], row['radius'], row['mass'], row['v_x'], row['v_y'])))
